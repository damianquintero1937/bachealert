  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        animation:"splash-bounce 0.8s ease infinite alternate",
      }}>🕳️</div>
      <div style={{ textAlign:"center" }}>
        <div style={{ color:"#FAFAF9", fontWeight:900, fontSize:28, letterSpacing:"-1px" }}>
          Bache<span style={{ color:"#F97316" }}>Alert</span>
        </div>
        <div style={{ color:"#78716C", fontSize:13, marginTop:4 }}>Cali, Valle del Cauca</div>
      </div>
      <div style={{ display:"flex", gap:6, marginTop:8 }}>
        {[0,1,2].map(i => (
          <div key={i} style={{
            width:8, height:8, borderRadius:"50%", background:"#F97316",
            animation:`splash-dot 1.2s ease ${i*0.2}s infinite alternate`,
            opacity:0.3,
          }} />
        ))}
      </div>
      <style>{`
        @keyframes splash-bounce { from{transform:scale(1)} to{transform:scale(1.08)} }
        @keyframes splash-dot { from{opacity:0.2} to{opacity:1} }
      `}</style>
    </div>
  );
}
...
// ─── Mapa ─────────────────────────────────────────────────────────────────────
function MapaReportes({ reportes, onMapClick, reporteSeleccionado, setReporteSeleccionado, ubicacionUsuario, modoReporte, filtroMapa, onCentrarUbicacion }) {
  ...
  useEffect(() => { modoReporteRef.current = modoReporte; }, [modoReporte]);
  ...
  useEffect(() => {
    ...
    const iniciarMapa = () => {
      const L = window.L;
      const centro = ubicacionRef.current ? [ubicacionRef.current.lat, ubicacionRef.current.lng] : [3.4516, -76.532];
      const zoom = ubicacionRef.current ? 16 : 14;
      const map = L.map(mapRef.current, { zoomControl: false }).setView(centro, zoom);
      ...
      map.on("click", (e) => { if (modoReporteRef.current) onMapClick({ lat:e.latlng.lat, lng:e.latlng.lng }); });
    };
    if (window.L) { iniciarMapa(); } else {
  ...
  // Centrar en ubicación cuando se presiona el botón
  const centrarEnUbicacion = () => {
    if (!ubicacionUsuario || !leafletMap.current) return;
    leafletMap.current.setView([ubicacionUsuario.lat, ubicacionUsuario.lng], 17);
  };
  useEffect(() => { if (onCentrarUbicacion) onCentrarUbicacion(centrarEnUbicacion); }, [ubicacionUsuario]);
  ...
  useEffect(() => {
    ...
    markersRef.current = [];
    const reportesFiltrados = filtroMapa === "todos" ? reportes : reportes.filter(r => r.tipo === filtroMapa);
  ...
  }, [reportes, reporteSeleccionado, filtroMapa]);

  return (
    <div style={{ position:"relative", width:"100%", height:"100%" }}>
      <style>{`
        @keyframes pulse{0%,100%{transform:scale(1);opacity:0.6}50%{transform:scale(2);opacity:0.1}}
        .leaflet-control-zoom { margin-bottom: 80px !important; margin-left: 12px !important; }
      `}</style>
...
function ModalReporte({ ubicacion, onClose, onSubmit, onCambiarUbicacion }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:9999, background:"rgba(0,0,0,0.8)", backdropFilter:"blur(4px)", display:"flex", alignItems:"flex-end", justifyContent:"center" }}>
      <div style={{ background:"#1C1917", borderRadius:"24px 24px 0 0", width:"100%", maxWidth:480, padding:"24px", paddingBottom:"max(24px, env(safe-area-inset-bottom))", maxHeight:"90vh", overflowY:"auto" }}>
        ...
        {paso === 3 ? (
          <div style={{ textAlign:"center", padding:"32px 0" }}>
            ...
            <p style={{ color:"#A8A29E", fontSize:14, marginBottom:28 }}>Ya aparece en el mapa. Compártelo con vecinos para sumar votos.</p>
            ...
            {paso === 2 && (
              <div>
                <div style={{ background:"#292524", borderRadius:10, padding:"10px 14px", marginBottom:14, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                  ...
                  <button onClick={onCambiarUbicacion} style={{ background:"#F9731622", border:"1px solid #F9731644", borderRadius:8, padding:"6px 10px", color:"#F97316", fontSize:11, fontWeight:700, cursor:"pointer", whiteSpace:"nowrap" }}>📍 Cambiar</button>
                </div>
...
function ModalResuelto({ reporte, onClose, onResuelto }) {
  ...
  const handleSubmit = async () => {
    if (!fotoFile) { alert("Necesitas subir una foto."); return; }
    setEnviando(true);
    try {
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:9999, background:"rgba(0,0,0,0.8)", backdropFilter:"blur(4px)", display:"flex", alignItems:"flex-end", justifyContent:"center" }}>
      <div style={{ background:"#1C1917", borderRadius:"24px 24px 0 0", width:"100%", maxWidth:480, padding:"24px", paddingBottom:"max(24px, env(safe-area-inset-bottom))", maxHeight:"80vh", overflowY:"auto" }}>
        <div style={{ width:48, height:4, background:"#44403C", borderRadius:99, margin:"0 auto 20px" }} />
        {listo ? (
          <div style={{ textAlign:"center", padding:"24px 0" }}>
            <div style={{ fontSize:56, marginBottom:12 }}>🎉</div>
            <h2 style={{ color:"#22C55E", fontSize:22, fontWeight:800, marginBottom:8 }}>¡Marcado como resuelto!</h2>
            <p style={{ color:"#A8A29E", fontSize:14, marginBottom:24 }}>El pin cambió a verde. ¡Gracias!</p>
          ...
          <>
            ...
            <p style={{ color:"#A8A29E", fontSize:14, marginBottom:20, lineHeight:1.6 }}>Sube una foto del arreglo como evidencia y el pin cambiará a 🟢.</p>
            <label style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:8, padding:"20px", borderRadius:12, border:`2px dashed ${fotoPreview?"#22C55E":"#44403C"}`, cursor:"pointer", marginBottom:16, background:fotoPreview?"#16A34A12":"transparent" }}>
              <input type="file" accept="image/*" capture="environment" onChange={handleFoto} style={{ display:"none" }} />
              {fotoPreview
                ? <><img src={fotoPreview} alt="preview" style={{ width:"100%", borderRadius:8, maxHeight:180, objectFit:"cover" }} /><span style={{ color:"#22C55E", fontWeight:600, fontSize:14 }}>📸 Foto lista</span></>
...
function PanelDetalle({ reporte, onClose, onVotar, onAbrirResuelto }) {
  ...
  return (
    <div style={{ position:"absolute", bottom:0, left:0, right:0, zIndex:1000, background:"#1C1917", borderRadius:"20px 20px 0 0", padding:"20px", paddingBottom:"max(20px, env(safe-area-inset-bottom))", boxShadow:"0 -4px 24px rgba(0,0,0,0.6)", maxHeight:"55vh", overflowY:"auto" }}>
      ...
      {!isResuelto && (
        <div style={{ background:"#292524", borderRadius:12, padding:"12px", marginBottom:12 }}>
          ...
          <div style={{ color:"#44403C", fontSize:11, marginTop:4 }}>
            ...
            {(reporte.votos||1)>=20&&(reporte.votos||1)<50&&"¡Alta presión! El pin es muy visible"}
            {(reporte.votos||1)>=50&&"🚨 Punto crítico — máxima visibilidad"}
          </div>
        </div>
      )}
      {!isResuelto && (
  ...
  useEffect(() => {
    const canal = supabase
      ...
      .on("postgres_changes", { event:"UPDATE", schema:"public", table:"reportes" }, (payload) => {
        setReportes(prev => prev.map(r => r.id === payload.new.id ? { ...r, ...payload.new } : r));
        setReporteSeleccionado(prev => prev?.id === payload.new.id ? { ...prev, ...payload.new } : prev);
      })
  ...
  return (
    <>
      ...
      <div style={{
        ...
        {/* Header */}
        <header style={{ background:"#1C1917", padding:"12px 16px", display:"flex", justifyContent:"space-between", alignItems:"center", borderBottom:"1px solid #292524", flexShrink:0, zIndex:100 }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ width:34, height:34, borderRadius:10, background:"linear-gradient(135deg, #F97316, #EA580C)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:17 }}>🕳️</div>
            <div>
              <div style={{ color:"#FAFAF9", fontWeight:900, fontSize:17, letterSpacing:"-0.5px" }}>Bache<span style={{ color:"#F97316" }}>Alert</span></div>
              <div style={{ color:"#78716C", fontSize:10 }}>
                Cali, Valle del Cauca
                {ubicacionUsuario && <span style={{ color:"#3B82F6", marginLeft:6 }}>📍 Activo</span>}
              </div>
            </div>
          </div>
          <div style={{ display:"flex", gap:6, alignItems:"center" }}>
            {cargando && <div style={{ color:"#78716C", fontSize:11 }}>...</div>}
            <div style={{ background:"#292524", borderRadius:10, padding:"5px 10px", color:"#F97316", fontSize:12, fontWeight:700 }}>{stats.total} reportes</div>
          </div>
        </header>

        {/* Banner modo reporte */}
        {modoReporte && (
          <div style={{ background:"#F97316", color:"white", textAlign:"center", padding:"10px", fontSize:14, fontWeight:700, flexShrink:0, zIndex:200 }}>
        ...
        <div style={{ display:"flex", background:"#1C1917", borderBottom:"1px solid #292524", flexShrink:0 }}>
          {[{ id:"mapa", label:"🗺️ Mapa" }, { id:"lista", label:"📋 Lista" }, { id:"stats", label:"📊 Cifras" }].map(tab => (
            <button key={tab.id} onClick={() => setVistaActiva(tab.id)} style={{ flex:1, padding:"11px 8px", border:"none", background:"transparent", color:vistaActiva===tab.id?"#F97316":"#78716C", fontWeight:vistaActiva===tab.id?700:500, fontSize:13, cursor:"pointer", borderBottom:vistaActiva===tab.id?"2px solid #F97316":"2px solid transparent" }}>
              {tab.label}
            </button>
          ))}
        </div>
            ...
            <div style={{ height:"100%", overflowY:"auto", padding:"16px" }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                ...
                {reportesFiltrados.map(r => {
                  ...
                  return (
                    <div key={r.id} onClick={() => { setReporteSeleccionado(r); setVistaActiva("mapa"); }} style={{ background:"#1C1917", borderRadius:14, overflow:"hidden", cursor:"pointer", border:"1px solid #292524" }}>
                      {/* Thumbnail foto */}
                      {r.foto_url && (
                        <img src={r.foto_url} alt="foto" style={{ width:"100%", height:120, objectFit:"cover", display:"block" }} />
                      )}
                      <div style={{ padding:"12px" }}>
                        <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
                          <div style={{ display:"flex", gap:8, alignItems:"center" }}>
                            <span style={{ fontSize:18 }}>{cfg.emoji}</span>
                            <span style={{ color:"#FAFAF9", fontWeight:700, fontSize:14 }}>{cfg.label}</span>
                          </div>
                        ...
                        <div style={{ color:"#78716C", fontSize:12, marginBottom:6 }}>📍 {r.barrio||"Cali"} · {r.created_at?new Date(r.created_at).toLocaleDateString("es-CO"):""}</div>
        ...
        {/* Botón flotante — posición segura en móvil */}
        {!modoReporte && (
          <button
